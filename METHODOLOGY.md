...
- Primary domain (for site-specific queries and passive probing)
- Subsidiary domains (optional)
- Assessment scope (which modules to run)

### Phase 2 — Passive OSINT Collection

Execute all six reconnaissance modules against the target:

| Module | Data Sources | Primary Targets |
|--------|-------------|-----------------|
| `dork-scan` | Google Custom Search | Exposed endpoints, PHI, configs, cloud storage |
| `llm-recon` | HTTP passive probe, Google | LLM APIs, prompt injection surface, governance gaps |
...
| `vendor-intel` | Google, public records | AI vendor relationships, supply chain |
| `social-recon` | LinkedIn, Google, job boards | Personnel disclosure, social engineering surface |
...
1. **Coverage is passive** — Active vulnerability scanning is out of scope. Findings represent externally visible exposure, not confirmed exploitability.
